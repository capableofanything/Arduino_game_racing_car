...
namespace KartGame.KartSystems
{
    public class ArcadeKart : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            if (m_CanMove)
            {
                if(serialController.rot[0] > 10.0 || serialController.rot[0] < -10.0)
                    turn = (float)serialController.rot[0] / 30.0f;

                if (turn > 1.0f)
                    turn = 1.0f;

                if (serialController.rot[1] < -5.0)
                {
                    brake = true;
                }
                ...
                MoveVehicle(acc, brake, turn);
                
                acc = false;
                brake = false;

                //MoveVehicle(Input.Accelerate, Input.Brake, Input.TurnInput);
                //Debug.Log("Acc : " + Input.Accelerate + "Brake : " + Input.Brake);
            }
            GroundAirbourne();
