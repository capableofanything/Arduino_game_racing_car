...
public class SerialController : MonoBehaviour
{
    ...
    public void DecryptMessage(string message)
    {
        ...
        onFoot = true;
        //Debug.Log("Entered DecryptMessage");
        string[] s = message.Split(',');
        if (s.Length <= 1) return;
        //Debug.Log("InputGyro : " + s[1] + ", " + s[2]);
        rot = new Vector3(float.Parse(s[1]), float.Parse(s[2]), 0.0f);
    }

    private void AssignSerialPort()
    {
        try
        {
            ...
            {
                Debug.Log("Device ID: " + serialPortName);
                Debug.Log("=======================================");
                ...
                serialThread = new SerialThreadLines(portName,
                                     ...
                                     reconnectionDelay,
                                     maxUnreadMessages);
                if(serialThread == null) { continue; }
                thread = new Thread(new ThreadStart(serialThread.RunForever));
                thread.Start();
                //OnEnable();
                if (ReferenceEquals((string)serialThread.ReadMessage(), SERIAL_DEVICE_CONNECTED))
                {
                    //messageListener.SendMessage("OnConnectionEvent", true);
                    break;
                }
        ...
        {
            Debug.LogError("Error getting USB port info: " + e.Message);
        }
    }
}
